...
export default function ThirdPartyPurchase(props) {
  ...
  const navItem = [
    ...
    ["Shipping Products", "/ThirdParty/ship"],
  ];
  React.useEffect(() => {
    (async () => {
      setLoading(true);
  ...
  const handleSetTxhash = async (id, hash) => {
    await supplyChainContract.methods
      ...
      .send({ from: roles.thirdparty, gas: 900000 });
  };
  ...
  return (
    <div classname={classes.pageWrap}>
      <Navbar titlePage={"Third Party"} navItems={navItem}>
        {loading ? (
          <Loader />
        ) : (
          <>
            ...
            <h1 className={classes.pageHeading}>Products</h1>
            <h3 className={classes.tableCount}>Total Products : {allProducts.length}</h3>
